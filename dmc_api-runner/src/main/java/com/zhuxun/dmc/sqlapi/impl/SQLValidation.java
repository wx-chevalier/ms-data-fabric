...
import com.zhuxun.dmc.sqlapi.errors.SQLValidationError;

public class SQLValidation {
  private static final WallConfig QUERY_ONLY_CONFIG = new WallConfig();
  private static final WallConfig RESTRICTED_CONFIG = new WallConfig();
  ...
  public void validate(String sql, String dialect) throws SQLValidationError {
    switch (dialect) {
      ...
      case "presto":
        ...
        break;
      default:
        throw new SQLValidationError("Unknown datasource type: " + dialect);
    }
  }
  ...
  private void checkWithWallProvider(WallProvider provider, String sql) throws SQLValidationError {
    ...
    if (result.getViolations().size() != 0) {
      ...
      throw new SQLValidationError(sql + ": " + errBuilder.toString());
    }
  }
}
