...
if [ ! -e ${PRESTO_NODE_DATA_DIR}/node.id ]; then
    cat /proc/sys/kernel/random/uuid > ${PRESTO_NODE_DATA_DIR}/node.id
fi

export PRESTO_NODE_ID=$(cat ${PRESTO_NODE_DATA_DIR}/node.id)
...
case "$1" in
    "coordinator" | "worker" )
        server_role="$1"
        shift
        exec launcher --config=${PRESTO_CONF_DIR}/${server_role}.properties "$@" run
        ;;
    *)
        ;;
esac
