 ...
 * @author Yan - tao
 */
public class ParameterValidateInstance implements OASValidate<List<QueryParameter>, Map<String,String>> {
    ...
    public void process(List<QueryParameter> queryParameters, Map<String,String> map) throws ValidateFailException {
        ...
        queryParameters.stream().filter(parameter -> parameter.getRequired() && "query".equals(parameter.getIn())).forEach(parameter -> {
            String name = parameter.getName();
            if (!map.containsKey(name)){
                throw new ValidateFailException("请求参数中,缺少参数:" + name);
            }
            ...
            if (parameter.getAllowEmptyValue()== null || !parameter.getAllowEmptyValue()){
                ...
                if (Strings.isNullOrEmpty(map.get(name))){
                    throw new ValidateFailException("参数:"+ name + "的值不允许为空");
                }
            }
        });
