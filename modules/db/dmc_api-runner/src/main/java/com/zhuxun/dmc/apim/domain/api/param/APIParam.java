...
public abstract class APIParam<T extends APIParam<T>> extends Modifiable<T> {
  public static APIParam of(DcApiParam dcApiParam, ObjectMapper objectMapper) throws IOException {
    ParamType type = ParamType.valueOf(dcApiParam.getParamType());
    switch (type) {
      case REQUEST_BODY:
        return new APIRequestBody()
            .setId(dcApiParam.getId())
            .setSchema(objectMapper.readValue(dcApiParam.getParamModel(), Schema.class));
      case PARAMETER:
        return new APIParameter()
            .setId(dcApiParam.getId())
            .setParameter(objectMapper.readValue(dcApiParam.getParamModel(), Parameter.class));
      default:
        throw new NotSupportedException("Unknown param type: " + dcApiParam.getParamType());
    }
  }
  ...
  public DcApiParam toEntity(ObjectMapper objectMapper) throws JsonProcessingException {
    ...
    return new DcApiParam().withParamType(getType().toString());
  }
