...
import static com.zhuxun.dmc.sqlapi.Utils.VARIABLE_INTERNAL_PREFIX;
...
public class VariableParser {
  ...
  /** 变量出现位置 -> 对应位置变量名 */
  @Getter List<Var> positionedVariables;

  private Set<String> SUPPORTED_TYPES =
      new HashSet<>(Arrays.asList("string", "integer", "number", "date", "datetime"));
  /** 当前解析的 SQL 文本 */
  ...
  /**
   * 替换括号定义的变量替换为 {@link #VARIABLE_INTERNAL_PREFIX} 后的语句
   *
   * <p>方便语法解析使用
   */
    ...
    while (this.ch != EOI) {
      switch (this.ch) {
        case '{':
          ...
          this.parsedSqlBuilder.append(VARIABLE_INTERNAL_PREFIX);
          ...
          switch (format) {
            ...
            default:
              throw new SQLAPIException(
                  format("Type not supported {type=%s, format=%s}", type, format));
          }
        default:
          throw new SQLAPIException(
              format("Type not supported {type=%s, format=%s}", type, format));
